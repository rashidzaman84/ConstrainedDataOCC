...
public class IncrementalRevBasedReplayerImpl5<M, C, S, T, L, A extends PartialAlignment<L, T, S>, P extends IncrementalRevBasedReplayerParametersImpl<M, L, T>>
	...
	private final Map<C, List<Double>> presentCosts;
	private final Map<C, List<Double>> forgottenCosts;
	private final Map<String, ArrayList<Triplet<String, Integer, Integer>>> eventsCategorisation;
	//private static int count=0;


	public IncrementalRevBasedReplayerImpl5(final S initialStateInModel, final S finalStateInModel,
		...
		this.presentCosts = new HashMap<>();
		this.forgottenCosts = new HashMap<>();
	...
	public Map<C, List<Double>> getCompoundCost() {
		Map<C, List<Double>> temp = new HashMap<>();
		for(Entry<C, List<Double>> entry : presentCosts.entrySet()) {
			if(forgottenCosts.containsKey(entry.getKey())) {
				double sum = sumArrayList(forgottenCosts.get(entry.getKey()));
				ArrayList<Double> temp2 = new ArrayList<>();
				temp2.add(sum+entry.getValue().get(0));
				temp.put(entry.getKey(), temp2);
				//presentCosts.put(entry.getKey(), entry.getValue().get(0)+sum);
			}
		}
		return temp;
	}
...
//		//verifyAlignment(alignment);
	...
	private void trackCosts(A alignment, C c) {
		/*if(compoundCost.containsKey(c)) {
			compoundCost.get(c).add(alignment.getCost());
		}else {
			List<Double> temp = new ArrayList<>();
			temp.add(alignment.getCost());
			compoundCost.put(c, temp);
		}*/
		List<Double> temp = new ArrayList<>();
		temp.add(alignment.getCost());
		presentCosts.put(c, temp);
	}
	...
	public String forgetCase() {	
		...
		C random = null;
		C caseToBeRemoved = null;
		boolean  priority2 = false;
		
		for(Entry<C, A> entry : dataStore.entrySet()) {
						
			if(entry.getValue().getCost()==0.0 && entry.getValue().size()>=parameters.getLookBackWindow()){
				...
				//continue;
			}else if(!priority2 && entry.getValue().getCost()>0.0 && entry.getValue().size()>=parameters.getLookBackWindow()){
				caseToBeRemoved = entry.getKey();
				priority2 = true;
		...
		if(caseToBeRemoved != null) {
			partiallyForgetTrace(caseToBeRemoved);
			System.out.println("CASE WAS FOUND");
		}else {
			System.out.println("Search for a case to be forgotten with required criteria not found");
			partiallyForgetTrace(random);
		}
		
		return (String) caseToBeRemoved;
		//return forgetCase_();
	}
	...
	private void partiallyForgetTrace(C c) {
		if(c.equals("178602")) {
			System.out.println("case 178602");
		}
		if(forgottenCosts.containsKey(c)){
			forgottenCosts.get(c).add(presentCosts.get(c).get(0));
			presentCosts.remove(c);
		}else {
			//System.out.println(c);
			ArrayList<Double> temp = new ArrayList<>();
			temp.add(presentCosts.get(c).get(0));
			forgottenCosts.put(c, temp);
			presentCosts.remove(c);
		}
