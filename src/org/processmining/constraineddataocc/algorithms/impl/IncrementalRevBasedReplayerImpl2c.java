...
public class IncrementalRevBasedReplayerImpl2c<M, C, S, T, L, A extends PartialAlignment<L, T, S>, P extends IncrementalRevBasedReplayerParametersImpl<M, L, T>>
implements IncrementalReplayer<M, C, S, T, L, A, P> {
	...
	public IncrementalRevBasedReplayerImpl2c(final S initialStateInModel, final S finalStateInModel,
	...
	public A processEvent(C c, L l) {
		...
		A previousAlignment = dataStore.remove(c);

		if (previousAlignment == null && dataStore.size() >= parameters.getMaxCasesToStore()) {
			forgetCase();
		}
	...
	private List<L> fetchTrace(final L newLabel, final A previousAlignment) {
	...
	protected A searchForNewAlignment(final C c, final L l, final A previousAlignment, final S state) {
		...
		final List<L> trace = fetchTrace(l, previousAlignment);
